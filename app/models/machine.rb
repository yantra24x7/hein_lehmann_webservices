...
#     :machine_status=>machine.machine_logs.last.present? ? machine.machine_logs.last.machine_status : nil,
     :last_data=>machine.external_machine_logs.last.present? ? machine.external_machine_logs.last.created_at.localtime.strftime("%d/%m/%Y ( %I:%M%p )") : nil,
     :machine_status=>machine.external_machine_logs.last.present? ? (Time.now - machine.external_machine_logs.last.created_at) > 600 ? false : machine.external_machine_logs.last.machine_status : nil
    } 
      end
end
